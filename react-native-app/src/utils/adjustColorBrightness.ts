...
const adjustColorBrightness = (color: string, percent: number) => {
  ...
  if (!/^#[0-9a-f]{6}$/i.test(color)) {
    throw new Error('Invalid color. Must be a hexadecimal color in the form #RRGGBB')
  }
  ...
  let r = Math.min(255, Math.max(0, (num >> 16) + (percent > 0 ? amount : -amount)))
  let b = Math.min(255, Math.max(0, ((num >> 8) & 0x00ff) + (percent > 0 ? amount : -amount)))
  let g = Math.min(255, Math.max(0, (num & 0x0000ff) + (percent > 0 ? amount : -amount)))
